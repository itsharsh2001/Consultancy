import classes from "./DocumentsRequired.module.css"
import Documents from "./ui/documents.js"

export default function DocumentsRequired() {
    return (
        <div>
            <h2 className={classes.p}>Documents required changing Object clause of MOA</h2>
        <div className={classes.documents}>
            <Documents image="Fintaximages/Documents/im1.png" text="Passport Size Photograph"/>
            <Documents image="Fintaximages/Documents/im2.png" text="MOA"/>
            <Documents image="Fintaximages/Documents/im3.png" text="AOA"/>
            <Documents image="Fintaximages/Documents/im4.png" text="Detail of change "/>
        </div>
        </div>
    )
}
